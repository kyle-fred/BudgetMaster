...
class EntityLookupServiceTest extends EntityLookupService {
  ...
  @Mock private JpaRepository<Budget, Long> mockRepository;

  @Mock private Income defaultIncome;
  ...
  class FindByIdOperations {
    ...
    void findByIdOrThrow_withExistingEntity_returnsEntity() {
      ...
      BudgetModelAssertions.assertBudget(result).isDefaultBudget();

      verify(mockRepository).findById(BudgetConstants.Default.ID);
    }
    ...
    void findByIdOrThrow_withNonExistentEntity_throwsException() {
      String errorMessage =
          String.format(ErrorConstants.Budget.NOT_FOUND_WITH_ID, BudgetConstants.Default.ID);

      when(mockRepository.findById(BudgetConstants.Default.ID)).thenReturn(Optional.empty());
  ...
  class FindListByCustomFinderOperations {
          ...
          findListByCustomFinderOrThrow(
              ...
              () ->
                  new IncomeNotFoundException(
                      String.format(
                          ErrorConstants.Income.NOT_FOUND_FOR_MONTH,
                          BudgetConstants.Default.YEAR_MONTH)));

      IncomeListAssertions.assertIncomes(result).hasSize(1).first().isDefaultIncome();
    }
    ...
    void findListByCustomFinderOrThrow_withNonExistentEntities_throwsException() {
      ...
      String errorMessage =
          String.format(
              ErrorConstants.Income.NOT_FOUND_FOR_MONTH, BudgetConstants.Default.YEAR_MONTH);
