...
class ExpenseServiceTest {
  ...
  private final ExpenseBudgetSynchronizer expenseBudgetSynchronizer =
      mock(ExpenseBudgetSynchronizer.class);
  private final ExpenseService expenseService =
      new ExpenseService(expenseRepository, expenseBudgetSynchronizer);
  ...
  class CreateExpenseOperations {
    ...
    void createExpense_withValidRequest_returnsCreated() {
      when(expenseRepository.saveAndFlush(any(Expense.class))).thenReturn(defaultExpense);
      doNothing().when(expenseBudgetSynchronizer).apply(any(Expense.class));

      Expense savedExpense = expenseService.createExpense(defaultExpenseRequest);

      ExpenseModelAssertions.assertExpense(savedExpense).isDefaultExpense();
  ...
  class GetExpenseOperations {
    ...
    void getAllExpensesForMonth_withValidMonth_returnsExpenses() {
      ...
      try (MockedStatic<DateUtils> mockedDateUtils = mockStatic(DateUtils.class)) {
        mockedDateUtils
            .when(() -> DateUtils.getValidYearMonth(ExpenseConstants.Default.YEAR_MONTH.toString()))
        ...
        ExpenseListAssertions.assertExpenses(result).hasSize(2).first().isDefaultExpense();
    ...
    void getExpenseById_withValidId_returnsExpense() {
      ...
      ExpenseModelAssertions.assertExpense(retrievedExpense).isDefaultExpense();
    ...
    void getAllExpensesForMonth_withNoExpenses_throwsException() {
      String errorMessage =
          String.format(
              ErrorConstants.Expense.NOT_FOUND_FOR_MONTH,
      ...
      try (MockedStatic<DateUtils> mockedDateUtils = mockStatic(DateUtils.class)) {
        ...
        ExpenseNotFoundException exception =
            assertThrows(
                ...
                () ->
                    expenseService.getAllExpensesForMonth(
                        ExpenseConstants.Default.YEAR_MONTH.toString()));
    ...
    void getExpenseById_withNonExistentId_throwsException() {
      String errorMessage =
          String.format(ErrorConstants.Expense.NOT_FOUND_WITH_ID, ExpenseConstants.NonExistent.ID);
  ...
  class UpdateExpenseOperations {
    ...
    void updateExpense_withValidRequest_returnsUpdated() {
      ...
      when(expenseRepository.findById(ExpenseConstants.Default.ID))
          .thenReturn(Optional.of(defaultExpense));
      doNothing().when(expenseBudgetSynchronizer).reapply(any(Expense.class), any(Expense.class));
      when(expenseRepository.saveAndFlush(any(Expense.class))).thenReturn(defaultExpense);

      Expense updatedExpense =
          expenseService.updateExpense(ExpenseConstants.Default.ID, updatedExpenseRequest);

      ExpenseModelAssertions.assertExpense(updatedExpense).isUpdatedExpense();
          ...
          assertThrows(
              ...
              () ->
                  expenseService.updateExpense(
                      ExpenseConstants.NonExistent.ID, defaultExpenseRequest));
  ...
  class DeleteExpenseOperations {
    ...
    void deleteExpense_withValidId_deletesExpense() {
      when(expenseRepository.findById(ExpenseConstants.Default.ID))
          .thenReturn(Optional.of(defaultExpense));
      doNothing().when(expenseBudgetSynchronizer).retract(any(Expense.class));
      doNothing().when(expenseRepository).deleteById(ExpenseConstants.Default.ID);
