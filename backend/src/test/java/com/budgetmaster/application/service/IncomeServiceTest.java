...
class IncomeServiceTest {
  ...
  private final IncomeBudgetSynchronizer incomeBudgetSynchronizer =
      mock(IncomeBudgetSynchronizer.class);
  private final IncomeService incomeService =
      new IncomeService(incomeRepository, incomeBudgetSynchronizer);
  ...
  class CreateIncomeOperations {
    ...
    void createIncome_withValidRequest_returnsCreated() {
      when(incomeRepository.saveAndFlush(any(Income.class))).thenReturn(defaultIncome);
      doNothing().when(incomeBudgetSynchronizer).apply(any(Income.class));

      Income savedIncome = incomeService.createIncome(defaultIncomeRequest);

      IncomeModelAssertions.assertIncome(savedIncome).isDefaultIncome();
  ...
  class GetIncomeOperations {
    ...
    void getAllIncomesForMonth_withValidMonth_returnsIncomes() {
      ...
      try (MockedStatic<DateUtils> mockedDateUtils = mockStatic(DateUtils.class)) {
        mockedDateUtils
            .when(() -> DateUtils.getValidYearMonth(IncomeConstants.Default.YEAR_MONTH.toString()))
            .thenReturn(IncomeConstants.Default.YEAR_MONTH);
        when(incomeRepository.findByMonth(IncomeConstants.Default.YEAR_MONTH)).thenReturn(incomes);

        List<Income> result =
            incomeService.getAllIncomesForMonth(IncomeConstants.Default.YEAR_MONTH.toString());

        IncomeListAssertions.assertIncomes(result).hasSize(2).first().isDefaultIncome();
    ...
    void getIncomeById_withValidId_returnsIncome() {
      ...
      IncomeModelAssertions.assertIncome(retrievedIncome).isDefaultIncome();
    ...
    void getAllIncomesForMonth_withNoIncomes_throwsException() {
      String errorMessage =
          String.format(
              ErrorConstants.Income.NOT_FOUND_FOR_MONTH, IncomeConstants.NonExistent.YEAR_MONTH);

      try (MockedStatic<DateUtils> mockedDateUtils = mockStatic(DateUtils.class)) {
        mockedDateUtils
            .when(
                () ->
                    DateUtils.getValidYearMonth(IncomeConstants.NonExistent.YEAR_MONTH.toString()))
            ...
            assertThrows(
                ...
                () ->
                    incomeService.getAllIncomesForMonth(
                        IncomeConstants.NonExistent.YEAR_MONTH.toString()));
    ...
    void getIncomeById_withNonExistentId_throwsException() {
      String errorMessage =
          String.format(ErrorConstants.Income.NOT_FOUND_WITH_ID, IncomeConstants.NonExistent.ID);

      when(incomeRepository.findById(IncomeConstants.NonExistent.ID)).thenReturn(Optional.empty());
  ...
  class UpdateIncomeOperations {
    ...
    void updateIncome_withValidRequest_returnsUpdated() {
      ...
      when(incomeRepository.findById(IncomeConstants.Default.ID))
          .thenReturn(Optional.of(defaultIncome));
      doNothing().when(incomeBudgetSynchronizer).reapply(any(Income.class), any(Income.class));
      when(incomeRepository.saveAndFlush(any(Income.class))).thenReturn(defaultIncome);

      Income updatedIncome = incomeService.updateIncome(IncomeConstants.Default.ID, updatedRequest);

      IncomeModelAssertions.assertIncome(updatedIncome).isUpdatedIncome();
  ...
  class DeleteIncomeOperations {
    ...
    void deleteIncome_withValidId_deletesIncome() {
      when(incomeRepository.findById(IncomeConstants.Default.ID))
          .thenReturn(Optional.of(defaultIncome));
      doNothing().when(incomeBudgetSynchronizer).retract(any(Income.class));
      doNothing().when(incomeRepository).deleteById(IncomeConstants.Default.ID);
