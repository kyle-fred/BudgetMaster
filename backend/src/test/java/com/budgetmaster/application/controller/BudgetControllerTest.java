...
class BudgetControllerTest {
  ...
  class GetBudgetOperations {
    ...
    void getBudget_withValidMonth_returnsOk() throws Exception {
      ...
      ResultActions validGetRequest =
          mockMvc.perform(
              get(PathConstants.Endpoints.BUDGET)
                  .param(
                      PathConstants.RequestParams.MONTH,
                      BudgetConstants.Default.YEAR_MONTH.toString()));

      BudgetControllerAssertions.assertThat(validGetRequest).isDefaultBudgetResponse();
      ...
      when(budgetService.getBudgetByMonth(BudgetConstants.NonExistent.YEAR_MONTH.toString()))
          .thenThrow(
              new BudgetNotFoundException(
                  String.format(
                      ErrorConstants.Budget.NOT_FOUND_FOR_MONTH,
                      BudgetConstants.NonExistent.YEAR_MONTH)));

      ResultActions nonExistentMonthRequest =
          mockMvc.perform(
              get(PathConstants.Endpoints.BUDGET)
                  .param(
                      PathConstants.RequestParams.MONTH,
                      BudgetConstants.NonExistent.YEAR_MONTH.toString()));
    ...
    void getBudget_withServiceError_returnsInternalServerError() throws Exception {
      ...
      BudgetControllerAssertions.assertThat(serviceErrorRequest).isInternalServerError();
  ...
  class DeleteBudgetOperations {
    ...
    void deleteBudget_withValidId_returnsNoContent() throws Exception {
      doNothing().when(budgetService).deleteBudget(BudgetConstants.Default.ID);

      ResultActions validDeleteRequest =
          mockMvc.perform(
              delete(PathConstants.Endpoints.BUDGET_WITH_ID, BudgetConstants.Default.ID));

      BudgetControllerAssertions.assertThat(validDeleteRequest).isNoContent();
    ...
    void deleteBudget_withNonExistentId_returnsNotFound() throws Exception {
      doThrow(
              new BudgetNotFoundException(
                  String.format(
                      ErrorConstants.Budget.NOT_FOUND_WITH_ID, BudgetConstants.NonExistent.ID)))
          .when(budgetService)
          .deleteBudget(BudgetConstants.NonExistent.ID);

      ResultActions nonExistentIdRequest =
          mockMvc.perform(
              delete(PathConstants.Endpoints.BUDGET_WITH_ID, BudgetConstants.NonExistent.ID));
