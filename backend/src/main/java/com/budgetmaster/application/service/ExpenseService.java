...
public class ExpenseService extends EntityLookupService {
  ...
  public ExpenseService(
      ExpenseRepository expenseRepository, ExpenseBudgetSynchronizer expenseBudgetSynchronizer) {
  ...
  public List<Expense> getAllExpensesForMonth(String monthString) {
    ...
    return findListByCustomFinderOrThrow(
        expenseRepository::findByMonth, month, createMonthNotFoundException(month));
  }

  public Expense getExpenseById(Long id) {
    return findByIdOrThrow(expenseRepository, id, createIdNotFoundException(id));
  }
  ...
  private Supplier<ExpenseNotFoundException> createIdNotFoundException(Long id) {
    return () ->
        new ExpenseNotFoundException(String.format(ErrorMessages.Expense.NOT_FOUND_WITH_ID, id));
  }

  /**
   * Creates a supplier for ExpenseNotFoundException when no entities are found for a given month
   * value.
   */
  private Supplier<ExpenseNotFoundException> createMonthNotFoundException(YearMonth month) {
    return () ->
        new ExpenseNotFoundException(
            String.format(ErrorMessages.Expense.NOT_FOUND_FOR_MONTH, month));
  }
}
