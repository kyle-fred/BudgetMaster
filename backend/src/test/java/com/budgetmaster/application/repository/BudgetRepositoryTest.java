...
class BudgetRepositoryTest {
  ...
  class BudgetPersistence {
    ...
    void save_withValidBudget_persistsBudget() {
      BudgetIntegrationAssertions.assertBudget(savedBudget).isDefaultBudget();
    }
  ...
  class BudgetRetrieval {
    ...
    void findById_withExistingId_returnsBudget() {
      ...
      BudgetIntegrationAssertions.assertBudget(foundBudget).isEqualTo(savedBudget);
    }
    ...
    void findByMonth_withExistingMonth_returnsBudget() {
      ...
      BudgetIntegrationAssertions.assertBudget(foundBudget.get()).isEqualTo(savedBudget);
    }

    @Test
    @DisplayName("Should return empty optional when budget not found")
    void findByMonth_withNonExistentMonth_returnsEmptyOptional() {
      Optional<Budget> foundBudget =
          budgetRepository.findByMonth(BudgetConstants.NonExistent.YEAR_MONTH);
