...
class IncomeRepositoryTest {
  ...
  class IncomeRetrieval {
    ...
    void findById_withExistingId_returnsIncome() {
      ...
      IncomeIntegrationAssertions.assertIncome(foundIncome).isEqualTo(savedIncome);
    }
    ...
    void findByMonth_withExistingMonth_returnsIncomes() {
      ...
      IncomeIntegrationListAssertions.assertIncomes(foundIncomes).hasSize(1).contains(savedIncome);
    }

    @Test
    @DisplayName("Should return empty list when no incomes found")
    void findByMonth_withNonExistentMonth_returnsEmptyList() {
      List<Income> foundIncomes =
          incomeRepository.findByMonth(IncomeConstants.NonExistent.YEAR_MONTH);

      IncomeIntegrationListAssertions.assertIncomes(foundIncomes).hasSize(0);
    }
  }
}
